...
public class TTS extends CordovaPlugin implements OnInitListener {
    ...
    private void speak(JSONArray args, CallbackContext callbackContext)
        ...
        // * [2018-08-30 14:46] I need to modify this part once the 'name' is provided
        if (hasName == true) {
            hasName = false;
            if (voices != null) {
                for (Voice voice: voices) {
                    if (Objects.equals(voice.getName(), name) ) {
                        tts.setVoice(voice);
                        hasName = true;
                        break;
                    }
    ...
    throws JSONException, NullPointerException {
        // * [2018-08-30 15:01] Get all the voices at first
        voices = tts.getVoices();
        // * [2018-08-30 15:06] Output it as a JSONARRAY
        JSONArray array = new JSONArray();
        if(voices!= null) {
            for (Voice voice : voices) {
                JSONObject entry = new JSONObject();
                entry.put("lang", voice.getLocale());
                entry.put("name", voice.getName());
                array.put(entry);
            }
        }
        // * [2018-08-30 16:22] Return the result
