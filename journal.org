* [2021-05-21 Fri] Plan
1. [10:37] Add a trash can for dots.
    1-1. I also correct an error for signIn2Async since I don't want to take isAuthLoaded$=>false into count.
2. [2021-06-17 Thu] record a video for OAuth processes
3. [2021-05-26 Wed] Don't guide this Web App to an App one. (comment the ~setTimeout~ part of ~init-browser-EventArgs~).
4. [2021-06-17 Thu] record actions and turn them to be gif.

* [2021-06-16 Wed] Cannot Work again
Does it mean that it might touch their red line?
Sigh, this is helping people. I still have more important job need to do. Forget this.

* [2021-06-05 Sat] Work

#+begin_src javascript
    setTimeout(async()=>{
      const hc = new Windows.Web.Http.HttpClient();
      const uri = new Windows.Foundation.Uri("https://www.youtube.com/get_video_info?html5=1&video_id=2vowHo2y-hI");
      const st = await hc.getStringAsync(uri);
      window.stt = st;
    },10);
#+end_src

* [2021-06-05 Sat] Dealing with YouTube's captions
1. ~MSApp~ is defined in ~TS 2.7~ as mentioned in [[https://stackoverflow.com/questions/51169240/msapp-equivalent-in-typescript-2-8][this StackOverflow's Q&A]].
    Copy part of it into my ~msapp.d.ts~ can make it work.
2. I should use ~@types/winrt-uwp~ instead of ~@types/winrt~.

* [2021-06-03 Thu] Using Future Access List to cache your media files in Windows system
1. ~FA.add~ is very interesting, if the ~StorageFile~ you added has been stored, it will move it to the first one of ~FA.entries~ even if you provide it with different ~metadata~; therefore, there is no item with multiple entries.
2. From its entries array, you just can get the information of ~metadata~ and ~token~ so that maybe I should use its file path as ~metadata~? 

* [2021-06-02 Wed] Correct Clipboard part & saveFile for Windows System

* [2021-06-01 Tue] Correct some bugs
1. For WinRT, I cannot use alert and confirm directly, I have to use ~Windows.UI.Popups.MessageDialog~.
2. For WinRT, ~Blob.text()~ does not support.
3. I still don't really understand how to update cached service worker.
4. Handle ~Windows.UI.WebUI.WebUIApplication.onactivated~ when the App is launching or has been launched. 
5. For WinRT, remember to add ~target="_blank"~ to open the link in a new window. By the way, in Angular, you cannot put this code at the tail of ~a~ or you might get error.
6. For WinRT, ~space-evenly~ is not supported.


* [2021-05-26 Wed] Try to change my windows 10 one to be a PWA one
Originall, in its ~package.windows10.appxmanifest~ its

    <Application Id="tw.at.yescirculation.mediadotpaper" StartPage="ms-appx-web://48028young-chunghsue.mediadotpaper/www/index.html">

I want to change its ~StartPage~ to be ~http://localhost:4200/index.html~.

* [2021-05-22 Sat] Correct two bugs
1. ~!!getFromGoogle["mimeType"]~ will throw an error when ~getFromGoogle~ is ~undefined~.
2. Since the code will import grobal variables from google drive after the user update their scopes, I have to avoid this behavior during importing and exporting.
3. rename yc.settings to settings.yescirculation.

* [2021-05-21 Fri] Now it can let user sign in and sign out.
1. Besides, it can toggle whether to share the settings with Google Drive.
2. Important:: For IIS, I have to set ~"scope":".";"start_url": "./" as mentioned in [[https://stackoverflow.com/questions/57993086/angular-pwa-in-iis-subfolder-not-work-offline][this StackOverflow's Q&A]].


* [2021-05-18 Tue] Bug fix:
1. For ~WithClickService~, it has three default HOFs.
    However, once they are created, their texts are also provided.
    Therefore, I provide this HOF an extra property ~viewData~ and use it data to open the dialog.
    It means that, I can update viewData when the texts are changed so that the texts in the open dialog is the recent one.
2. Because of CORS, for web version, I have to use ~gapi.files.get~ to get a Google Drive's file.
    The Bug I fixed is to remind the user that the file they are loaded is a media file if it is a media file to remind them to share it if they want to share this mdpyc with others.

* [2021-05-17 Mon] Make Line can show my links better
1. To achieve this goal, I need to add some meta tags. We can easier make that through the help of [[https://metatags.io]]. Remember to declare the width and height of the image.
2. Using SWUpdate to update [[https://angular.io/guide/service-worker-intro][service worker]].

* [2021-05-15 Sat] Since each user has their own settings, I set a global variable "googleUsers" to store this information.
1. Because for each user, their export2FolderId will not be the same, I put this value inside the array "googleUsers".
2. Now, once the user login, it will ask them whether to export a setting file to google drive.
3. Because the user name might be the same, I also get the user id to distinguish between them.

* [2021-05-14 Fri] The files that I need to modify for multilingual
Its definition file is [[src/IPageTexts.d.ts]].
From this file, seaching one of its key globally, you'll find out all files that need to be modified.

* [2021-05-14 Fri] Dealing with multilingual
Some parts that I still have no idea how to make them multilingual.
I marked them with ~//I18n~.

* [2021-05-11 Tue] Migrate this App to Google Workspace.
1. Take care of the quota.
2. ~auth/drive.file~ is for the files created by this App and ~auth/drive.readonly~ is for all files so that ~auth/drive.readonly~ is more sensitive.
3. Looks like I don't need to ask for accessing all files so that I don't need to include ~auth/drive~ at this moment.
4. Once the user is changed, this App will import the settings from google drive since that data should be changed when the user is changed.
5. Once the media file is checked as a video or an audio, change the display of ~video~ and ~audio~ HTMLElements.

* [2021-04-27 Tue] PWA can work now
1. I need to deal with subfolder's problem. You can follow this [[https://forum.ionicframework.com/t/how-to-configure-a-pwa-to-run-on-a-subfolder-ask-question/160517/5][Q&A]]
to solve it.
`
    ng build --prod --base-href=/static/mediadotpaper/
`
2. Sometimes, it does not load service worker immediately, although [[https://stackoverflow.com/questions/60316067/angular-v8-throw-error-no-matching-service-worker-detected][Q&A]]
might solve this problem, but keeping its default setting might be better.

* [2021-04-26 Mon] I have tried to show dialogs for click events for the GAPI service
