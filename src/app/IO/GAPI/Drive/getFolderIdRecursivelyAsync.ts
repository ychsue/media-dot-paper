...
async function getFolderIdRecursivelyGAPIAsync({ name, parentId }
...
export default async function getFolderIdRecursivelyAsync({ name, grantWithClick, signInWithClick, parentId }
    : { name: string, grantWithClick?: tHOF, signInWithClick?: tHOF, parentId?: string }) {
    const scopes = "https://www.googleapis.com/auth/drive";
    var res = await withMustSignIn({ scopes, grantWithClick, signInWithClick })(getFolderIdRecursivelyGAPIAsync)({ name, parentId });
