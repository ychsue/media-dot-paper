...
export class DbService {
  ...
  constructor(
    private cService: DeviceService,
    ...
    const subscriber = (observer) => {
      const action = async () => {
        if (cService.isCordova) {
          await cService.onDeviceReady.toPromise();
        }
  ...
  async iniNanoSQL(ob: Subscriber<boolean>) {
    const mode =
      !!window["nSQLite"] && window.cordova.platformId !== "browser"
        ? window["nSQLite"].getMode()
        : "IDB";
    ...
    let rows: any;
    rows = await nSQL(DbService.storyTableName)
  ...
  async searchAsync(
    tName: string = DbService.storyTableName,
    selOptions?: any,
    where?: any,
    orderBy?: any,
    offset?: number,
